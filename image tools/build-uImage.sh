...
cd $INITRAMFSDIR
find * | cpio -C 1 -R root:root -H newc -o | gzip > ../$WORKDIR/initramfs.new.cpio
cd ..
cp ${UIMAGE} $WORKDIR
cp mkimage $WORKDIR
cd $WORKDIR
...
./mkimage -A arm -O linux -T kernel -C lzma -a 80008000 -e 80008000 -d Image.new.lzma -n MSRWork uImage.new
#
